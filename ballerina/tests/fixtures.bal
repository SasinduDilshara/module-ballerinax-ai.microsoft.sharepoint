...
// verified to round-trip through Tika's PDF parser, so the loader returns an
...
// The marker text Tika extracts from the PDF fixture above. This loader does not support
// Microsoft Office documents (.doc/.docx/.ppt/.pptx/.xls/.xlsx) — they are skipped like
// unsupported files — so no Office fixtures or markers are needed here.
const string PDF_TEXT = "Mock PDF document text.";

// Returns the matching extractable fixture bytes for a file extension, or `()`.
isolated function extractableFixture(string extension) returns byte[]? {
    match extension {
        "pdf" => { return PDF_BYTES; }
    }
    return ();
}

// The marker text expected from the extractable fixture for a file extension, or `()`.
isolated function extractableText(string extension) returns string? {
    match extension {
        "pdf" => { return PDF_TEXT; }
    }
    return ();
